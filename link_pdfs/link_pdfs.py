...
def extract_folder_name(subfolder):
    '''
    From file_path/SSC-MS-00559 to SSC-MS-00559
    '''
...
def make_matches(subfolders, resources):
...
def check_ao_for_note(ao, file_path): # This is for a singular archival object
    check = 0
...
def update_archival_object_record(match): 
    successful_files = []
...
def update_resource_with_note(match_resource_uri):
    '''Adds Other Finding Aid note to Resource record if none exists'''
    rec = aspace.get(match_resource_uri)
...
if __name__ == "__main__":
    ...
    # File generated from a SQL query
    ...
    data['rows'].extend(make_matches(subfolders, resources))
    logging.info('Matches made!')

    # Writing to JSON file  
    with open('.matches_cache.json', 'w') as outfile:
        json.dump(data, outfile)


    # Comment out above code if only need to run second half 
    # Attaching notes and updating ASpace records
    with open('.file_matches.json') as json_file:
        try:
    ...
    for match in matches['rows']:
        try:
            file_update = update_archival_object_record(match)
            if file_update != None:
                d = {}
                d['directory'] = match['directory']
                d['resource_uri'] = match['resource_uri']
                d['misses'] = file_update # List of unmatched files
                unmatched['rows'].append(d)
        except Exception as e:
            logging.error(e)
        
        try:
            resource_update = update_resource_with_note(match['resource_uri'])
        except Exception as e:
            logging.error(e)

    
    with open('.unlinked_pdfs.json', 'w') as outfile:
        json.dump(unmatched, outfile)
        

    logging.info('All done! Check unliked_pdfs.json for problem PDFs.')
